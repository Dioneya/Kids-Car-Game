			...
			bool CheckOriginalSpriteTexture (float2 uv, bool ifZero)
			{
				...
				else
				{
					outline = 	SampleSpriteTexture(uv + fixed2(0, +thicknessY)).a > alphaThreshold ||
								...
								SampleSpriteTexture(uv + fixed2(-thicknessX, 0)).a > alphaThreshold ||
								SampleSpriteTexture(uv + fixed2(+thicknessX * 0.999906, -thicknessY * 0.0137)).a > alphaThreshold ||
								SampleSpriteTexture(uv + fixed2(-thicknessX * 0.999906, +thicknessY * 0.0137)).a > alphaThreshold ||
								SampleSpriteTexture(uv + fixed2(-thicknessX * 0.999906, -thicknessY * 0.0137)).a > alphaThreshold ||
								SampleSpriteTexture(uv + fixed2(+thicknessX * 0.999906 /*cos (3.14 / 4)*/, +thicknessY * 0.0137 /*sin (3.14 / 4)*/)).a > alphaThreshold;
				}
				if(outline) return outline;

				float angle = steps * angle_step * 2 * 3.14 / 360;
